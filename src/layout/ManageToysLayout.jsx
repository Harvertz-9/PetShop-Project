        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
                ...
                <div className="flex items-center justify-between px-8 pt-7 pb-5 border-b border-surface-container-high">
                    ...
                    <button onClick={onClose} className="w-9 h-9 rounded-full hover:bg-surface-container-low flex items-center justify-center transition-all">
                ...
                <form onSubmit={handleSubmit} className="px-8 py-6 space-y-5">
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="flex gap-1 bg-surface-container-low p-1 rounded-xl">
                            <button type="button" onClick={() => setImgTab("url")}
                                className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${imgTab === "url" ? "bg-white shadow text-on-surface" : "text-on-surface-variant"}`}>
                                Image URL
                            </button>
                            <button type="button" onClick={() => setImgTab("file")}
                                className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${imgTab === "file" ? "bg-white shadow text-on-surface" : "text-on-surface-variant"}`}>
                                Upload File
                            </button>
                        </div>
                    ...
                    <div className="space-y-1.5">
                        ...
                        <input
                            type="text"
                            placeholder="e.g. Small - Large sizes"
                            value={form?.desc || ""}
                    ...
                    <div className="space-y-1.5">
                        ...
                        <input
                            type="text"
                            placeholder="e.g. Bestseller, New, Eco Friendly"
                            value={form?.badge || ""}
                    ...
                    <div className="flex gap-3 pt-2">
                        <button type="button" onClick={onClose}
                            className="flex-1 py-3.5 bg-surface-container-low rounded-xl font-bold text-sm hover:bg-surface-container transition-all">
                            Cancel
                        </button>
                        <button type="submit"
                            className="flex-1 py-3.5 bg-gradient-to-r from-primary to-primary-container text-on-primary rounded-xl font-black text-sm shadow-lg hover:opacity-90 active:scale-[0.98] transition-all">
                            Save Product
                        </button>
                    </div>
                </form>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm p-8 text-center">
                ...
                <div className="flex gap-3">
                    <button onClick={onClose} className="flex-1 py-3 bg-surface-container-low rounded-xl font-bold text-sm hover:bg-surface-container transition-all">
                        Cancel
                    </button>
                    <button onClick={onConfirm} className="flex-1 py-3 bg-error text-white rounded-xl font-black text-sm hover:opacity-90 active:scale-95 transition-all">
                        Delete
                    </button>
                </div>
            </div>
...
export default function ManageToysLayout() {
    ...
    const toyProducts = products.filter(p =>
        ...
        p.name.toLowerCase().includes('rubber') ||
        p.name.toLowerCase().includes('ball') ||
        p.name.toLowerCase().includes('rope')
    )

    const defaultForm = { name: "", price: "", category: "Eco Toys", desc: "", longDesc: "", src: "", badge: null }
    ...
    const handleAdd = (data) => {
        addProduct({ ...data, category: "Eco Toys" })
    }
    ...
    return (
        <>
                ...
                <main className="flex-1 min-w-0">
                    <div className="p-8 max-w-7xl mx-auto space-y-8">
                        ...
                        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                            <div>
                                <h1 className="text-5xl font-extrabold text-on-surface tracking-tight">Manage Toys</h1>
                                <p className="text-on-surface-variant mt-1">{toyProducts.length} products in catalog</p>
                            </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            {[
                                { label: "Total Toys", value: toyProducts.length, icon: "toys", color: "text-primary" },
                                { label: "Avg Price", value: toyProducts.length ? `$${(toyProducts.reduce((s, p) => s + p.price, 0) / toyProducts.length).toFixed(2)}` : "$0", icon: "payments", color: "text-secondary" },
                                { label: "Categories", value: [...new Set(toyProducts.map(p => p.category))].length, icon: "category", color: "text-tertiary" }
                            ].map((s, i) => (
                        ...
                        <div className="bg-surface-container-lowest rounded-2xl shadow-[0_4px_24px_rgba(25,28,29,0.06)] overflow-hidden">
                                        ...
                                        {toyProducts.length === 0 ? (
                                            <tr>
                                                <td colSpan={5} className="px-6 py-16 text-center text-on-surface-variant">
                                                    <span className="material-symbols-outlined text-5xl block mb-3">toys</span>
                                                    No toy products yet. Click "Add New Toy" to get started.
                                                </td>
                                            </tr>
                                                ...
                                                <td className="px-6 py-4">
                                                    <div className="flex justify-end gap-1">
                                                        <button
                                                            ...
                                                            className="w-9 h-9 rounded-full flex items-center justify-center text-on-surface-variant hover:bg-primary/10 hover:text-primary transition-all"
                                                            title="Edit">
                                                            <span className="material-symbols-outlined text-lg">edit</span>
                                                        </button>
                                                        <button
                                                            onClick={() => { setSelectedProduct(product); setDeleteOpen(true) }}
                                                            className="w-9 h-9 rounded-full flex items-center justify-center text-on-surface-variant hover:bg-red-50 hover:text-error transition-all"
                                                            title="Delete">
                            ...
                            {toyProducts.length > 0 && (
                                <div className="px-6 py-4 border-t border-surface-container-high">
                                    <p className="text-xs text-on-surface-variant font-medium">Showing {toyProducts.length} toy product{toyProducts.length !== 1 ? "s" : ""}</p>
                                </div>
                            )}
                        </div>
                    </div>
            ...
            {/* Modals */}
            <ProductModal isOpen={addOpen} onClose={() => setAddOpen(false)} onSave={handleAdd} initialData={defaultForm} title="Add New Toy Product" />
            <ProductModal isOpen={editOpen} onClose={() => { setEditOpen(false); setSelectedProduct(null) }} onSave={handleEdit} initialData={selectedProduct || defaultForm} title="Edit Toy Product" />
